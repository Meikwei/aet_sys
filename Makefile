...
PROJECT_NAME := "github.com/Meikwei/aet_sys"
PKG := "$(PROJECT_NAME)"
...
graph:
	...
	@cp -f cmd/sys/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=sys github.com/Meikwei/aet_sys
	@rm -f main.go sys.gv
...
.PHONY: build
# build sys for linux amd64 binary
build:
	@echo "building 'sys', linux binary file will output to 'cmd/sys'"
	@cd cmd/sys && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
