...
type SysRoleDao interface {
	...
	GetByCondition(ctx context.Context, condition *query.Conditions) (*model.SysRole, error)
	GetByIDs(ctx context.Context, ids []uint64) (map[uint64]*model.SysRole, error)
	GetByLastID(ctx context.Context, lastID uint64, limit int, sort string) ([]*model.SysRole, error)
...
func (d *sysRoleDao) DeleteByIDs(ctx context.Context, ids []uint64) error {
	err := d.db.WithContext(ctx).Where("id IN (?)", ids).Delete(&model.SysRole{}).Error
	if err != nil {
		return err
	}
...
func (d *sysRoleDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.SysRole) error {
	...
	if table.CreateUser != "" {
		update["create_user"] = table.CreateUser
	}
	if table.UpdateUser != "" {
		update["update_user"] = table.UpdateUser
	}
...
func (d *sysRoleDao) GetByID(ctx context.Context, id uint64) (*model.SysRole, error) {
	...
	if errors.Is(err, model.ErrCacheNotFound) {
		...
		val, err, _ := d.sfg.Do(utils.Uint64ToStr(id), func() (interface{}, error) { //nolint
			...
			// set cache
			err = d.cache.Set(ctx, id, table, cache.SysRoleExpireTime)
			if err != nil {
...
//		},
//	}
func (d *sysRoleDao) GetByCondition(ctx context.Context, c *query.Conditions) (*model.SysRole, error) {
...
func (d *sysRoleDao) GetByIDs(ctx context.Context, ids []uint64) (map[uint64]*model.SysRole, error) {
	// no cache
	if d.cache == nil {
		...
		itemMap := make(map[uint64]*model.SysRole)
	...
	if len(missedIDs) > 0 {
		...
		if len(realMissedIDs) > 0 {
			...
			if len(missedData) > 0 {
				...
				err = d.cache.MultiSet(ctx, missedData, cache.SysRoleExpireTime)
				if err != nil {
					return nil, err
				}
			} else {
...
func (d *sysRoleDao) GetByLastID(ctx context.Context, lastID uint64, limit int, sort string) ([]*model.SysRole, error) {
...
func (d *sysRoleDao) GetByColumns(ctx context.Context, params *query.Params) ([]*model.SysRole, int64, error) {
	...
	if params.Sort != "ignore count" { // determine if count is required
		err = d.db.WithContext(ctx).Model(&model.SysRole{}).Select([]string{"id"}).Where(queryStr, args...).Count(&total).Error
		if err != nil {
			return nil, 0, err
		}
		if total == 0 {
...
func (d *sysRoleDao) DeleteByTx(ctx context.Context, tx *gorm.DB, id uint64) error {
	...
	err := tx.WithContext(ctx).Model(&model.SysRole{}).Where("id = ?", id).Updates(update).Error
	if err != nil {
		return err
	}
